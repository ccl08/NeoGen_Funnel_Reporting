...
-- 2. Build the KPI Funnel Table
CREATE OR REPLACE TABLE `neogen-ga4-export.funnelPurchase_table.kpi_funnel` AS
...
WITH raw_events AS (
  ...
  WHERE
    _TABLE_SUFFIX BETWEEN '20260320' AND FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))
...
-- Wide aggregation: one row per date + dimensions, columns per segment
agg AS (
  SELECT
    date,
    ...
    COUNT(DISTINCT IF(is_overall AND has_atc, session_id, NULL))            AS overall_atc,
    COUNT(DISTINCT IF(is_overall AND has_view_cart, session_id, NULL))      AS overall_view_cart,
    COUNT(DISTINCT IF(is_overall AND has_checkout, session_id, NULL))       AS overall_checkout,
    COUNT(DISTINCT IF(is_overall AND has_purchase, session_id, NULL))       AS overall_purchase,

    -- PDP
    COUNT(DISTINCT IF(is_pdp, session_id, NULL))                            AS pdp_sessions,
    COUNT(DISTINCT IF(is_pdp AND has_atc, session_id, NULL))                AS pdp_atc,
    COUNT(DISTINCT IF(is_pdp AND has_view_cart, session_id, NULL))          AS pdp_view_cart,
    COUNT(DISTINCT IF(is_pdp AND has_checkout, session_id, NULL))           AS pdp_checkout,
    COUNT(DISTINCT IF(is_pdp AND has_purchase, session_id, NULL))           AS pdp_purchase,

    -- SEARCH
    COUNT(DISTINCT IF(is_search, session_id, NULL))                         AS search_sessions,
    COUNT(DISTINCT IF(is_search AND has_atc, session_id, NULL))             AS search_atc,
    COUNT(DISTINCT IF(is_search AND has_view_cart, session_id, NULL))       AS search_view_cart,
    COUNT(DISTINCT IF(is_search AND has_checkout, session_id, NULL))        AS search_checkout,
    COUNT(DISTINCT IF(is_search AND has_purchase, session_id, NULL))        AS search_purchase,

    -- PCP
    COUNT(DISTINCT IF(is_pcp, session_id, NULL))                            AS pcp_sessions,
    COUNT(DISTINCT IF(is_pcp AND has_atc, session_id, NULL))                AS pcp_atc,
    COUNT(DISTINCT IF(is_pcp AND has_view_cart, session_id, NULL))          AS pcp_view_cart,
    COUNT(DISTINCT IF(is_pcp AND has_checkout, session_id, NULL))           AS pcp_checkout,
    COUNT(DISTINCT IF(is_pcp AND has_purchase, session_id, NULL))           AS pcp_purchase,

    -- MY ACCOUNT
    COUNT(DISTINCT IF(is_my_account, session_id, NULL))                     AS accounts_sessions,
    COUNT(DISTINCT IF(is_my_account AND has_atc, session_id, NULL))         AS accounts_atc,
    COUNT(DISTINCT IF(is_my_account AND has_view_cart, session_id, NULL))   AS accounts_view_cart,
    COUNT(DISTINCT IF(is_my_account AND has_checkout, session_id, NULL))    AS accounts_checkout,
    COUNT(DISTINCT IF(is_my_account AND has_purchase, session_id, NULL))    AS accounts_purchase
